...
echo "[1/9] Systeem pakketlijsten verversen en updates installeren..."
bash "${SCRIPT_DIR}/scripts/01_system/01_system_update.sh"

echo "[2/9] Tijd en datum synchroniseren + timezone Europe/Amsterdam instellen..."
bash "${SCRIPT_DIR}/scripts/01_system/02_set_time_and_timezone.sh"

echo "[3/9] SSH pakketten installeren..."
bash "${SCRIPT_DIR}/scripts/02_ssh/03_install_ssh_packages.sh"

echo "[4/9] Gebruiker '${MICHAEL_USER}' configureren..."
bash "${SCRIPT_DIR}/scripts/02_ssh/04_configure_michael_user.sh"

echo "[5/9] Root login uitschakelen en SSH hardenen..."
bash "${SCRIPT_DIR}/scripts/02_ssh/05_harden_ssh.sh"

echo "[6/9] Firewall pakketten installeren..."
bash "${SCRIPT_DIR}/scripts/03_firewall/06_install_firewall_packages.sh"

echo "[7/9] Firewall regels met chain 'ip' instellen..."
bash "${SCRIPT_DIR}/scripts/03_firewall/07_configure_firewall.sh"

echo "[8/9] Controleren of alles goed is ingesteld (en zo nodig herstellen)..."
bash "${SCRIPT_DIR}/scripts/04_system/10_verify_and_repair.sh"

echo "[9/9] Opschonen van ongebruikte pakketten..."
