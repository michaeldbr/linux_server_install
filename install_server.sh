...
echo "[3/11] SSH pakketten installeren..."
"${SCRIPT_DIR}/scripts/02_ssh/03_install_ssh_packages.sh"

echo "[4/11] Gebruiker '${MICHAEL_USER}' configureren..."
"${SCRIPT_DIR}/scripts/02_ssh/04_configure_michael_user.sh"

echo "[5/11] Root login uitschakelen en SSH hardenen..."
"${SCRIPT_DIR}/scripts/02_ssh/05_harden_ssh.sh"

echo "[6/11] Firewall pakketten installeren..."
"${SCRIPT_DIR}/scripts/03_firewall/06_install_firewall_packages.sh"

echo "[7/11] Firewall regels met chain 'ip' instellen..."
"${SCRIPT_DIR}/scripts/03_firewall/07_configure_firewall.sh"

echo "[8/11] WireGuard installeren..."
"${SCRIPT_DIR}/scripts/03_firewall/09_install_wireguard.sh"

echo "[9/11] HAProxy, Keepalived en Kubernetes master-pakketten installeren..."
"${SCRIPT_DIR}/scripts/05_kubernetes/08_install_cluster_packages.sh"
...
echo "Klaar. SSH draait op poort ${SSH_PORT}. WireGuard luistert op UDP poort ${WIREGUARD_PORT}. Tijdzone staat op Europe/Amsterdam. HAProxy, Keepalived en Kubernetes master-pakketten zijn geïnstalleerd."
